...
class DateConverterHelper {
  ...
  static String dateTimeStringToShortDate(String dateTime) {
    return DateFormat('d MMM, y').format(dateTimeStringToDate(dateTime));
  }

  static String dateTimeStringToTimeOnly(String dateTime) {
    return DateFormat('hh:mm a').format(dateTimeStringToDate(dateTime));
  }
  ...
  static String utcToDateTime(String dateTime) {
    ...
    parsedDate ??= DateTime.tryParse(dateTime.replaceFirst(' ', 'T'));

    if (parsedDate != null) {
      return DateFormat('dd MMM, yyyy h:mm a').format(parsedDate.toLocal());
    }

    String cleanedDate = dateTime.replaceAll('T', ' ');
    cleanedDate = cleanedDate.replaceAll(RegExp(r'(\.\d+)?Z$', caseSensitive: false), '');
    return cleanedDate.trim();
  }
  ...
  static String orderEditLogDateTime(String dateTime) {
    DateTime parsed = DateTime.tryParse(dateTime)?.toLocal() ?? dateTimeStringToDate(dateTime);
