...
class _CategoryProductScreenState extends State<CategoryProductScreen> {
  ...
  void initState() {
    ...
    Get.find<CategoryController>().getSubCategoryList(widget.categoryId, isRestaurantWise: true);
    ...
    scrollController.addListener(() {
      ...
      final bool listLoaded = categoryController.isServiceModule ? categoryController.serviceItemList != null : categoryController.itemList != null;
      if (scrollController.position.pixels == scrollController.position.maxScrollExtent
          && listLoaded && !Get.find<CategoryController>().isLoading) {
      ...
      body: GetBuilder<CategoryController>(builder: (categoryController) {
        return SingleChildScrollView(
          ...
          child: Column(children: [
            ...
            () {
              final bool isServiceModule = categoryController.isServiceModule;
              final bool listLoaded = isServiceModule ? categoryController.serviceItemList != null : categoryController.itemList != null;
              final int listLength = isServiceModule ? (categoryController.serviceItemList?.length ?? 0) : (categoryController.itemList?.length ?? 0);

              return listLoaded ? listLength > 0 ? GridView.builder(
                key: UniqueKey(),
                ...
                itemBuilder: (context, index) {
                  return Padding(
                    ...
                    child: isServiceModule ? ServiceWidget(
                      service: categoryController.serviceItemList![index],
                      index: index, length: listLength,
                    ) : ItemWidget(
                      item: categoryController.itemList![index],
                      index: index, length: listLength, isCampaign: false,
                      inStore: true,
                    ),
                  );
                },
                ...
                itemBuilder: (context, index) {
                  return ItemShimmerWidget(
                    isEnabled: !listLoaded, hasDivider: index != 19,
                  );
                },
              );
            } (),
