...
class CustomTimePickerWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    bool isService = addressController.moduleList != null && addressController.selectedModuleIndex != -1 &&
        addressController.moduleList![addressController.selectedModuleIndex!].moduleType == 'service';
      ...
      child: Container(
        ...
        child: GetBuilder<AuthController>(builder: (authController) {
          ...
          return Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text(
              isService ? 'estimated_service_time'.tr : (isRental ? 'estimated_pickup_time_time'.tr : 'estimated_delivery_time'.tr),
            ...
            Padding(
              ...
              child: Text(
                (isService ? 'this_service_will_be_shown_in_the_user_app_website' : 'this_item_will_be_shown_in_the_user_app_website').tr,
            ...
            CustomButtonWidget(
              ...
              onPressed: (){
                ...
                if(min == null){
                  showCustomSnackBar((isService ? 'minimum_service_time_can_not_be_empty' : (isRental ? 'minimum_pickup_time_can_not_be_empty' : 'minimum_delivery_time_can_not_be_empty')).tr);
                }else if(max == null){
                  showCustomSnackBar((isService ? 'maximum_service_time_can_not_be_empty' : (isRental ? 'maximum_pickup_time_can_not_be_empty' : 'maximum_delivery_time_can_not_be_empty')).tr);
                }else if(authController.storeTimeUnit.isEmpty){
                  showCustomSnackBar('time_unit_can_not_be_empty'.tr);
                }else if(min < max){
                  Get.back();
                }else{
                  showCustomSnackBar((isService ? 'maximum_service_time_can_not_be_smaller_then_minimum_service_time' : (isRental ? 'maximum_pickup_time_can_not_be_smaller_then_minimum_pickup_time' : 'maximum_delivery_time_can_not_be_smaller_then_minimum_delivery_time')).tr);
                }
              },
            ),
